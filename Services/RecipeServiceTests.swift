...
class RecipeServiceTests: XCTestCase {
    ...
    func testFetchDesserts() throws {
        let expectation = self.expectation(description: "Fetching Desserts")
        
        recipeService.fetchDesserts {
            ...
            case .failure(let error): XCTFail("Error fetching desserts: \(error)")
            }
            expectation.fulfill()
        }
    ...
    func testFetchRecipeDetail() throws {
        let expectation = self.expectation(description: "Fetching recipe Detail")
        ...
        recipeService.fetchRecipeDetail(id: sampleID) { (result) in switch result {
        ...
        case .failure(let error): XCTFail("Error fetching recipe details: \(error)")
        }
            expectation.fulfill()
        }
